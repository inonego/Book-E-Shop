...
#pragma region °ü¸®ÀÚ ¸Þ´ºÈ­¸é
	...
	// »óÇ° ¸ñ·Ï ¸Þ´ºÈ­¸é
	{
		...
		_template.show_func = [](Product* product) -> string {
			return format("{0:<10}{1:<20}{2:<8}{3:<8}{4:<12}{5:<8}", product->id, limit(product->title, 18), product->genre, product->author, to_string(product->price) + "¿ø", product->count);
		};
	...
	menu_manager->AppendMenu(MENU_A_PRODUCT_INFO, new Menu<Product*>(
		[=](MenuIO& IO, Product* target) { 
			...
			IO.print("°ø¹éÀ» ÀÔ·ÂÇÒ °æ¿ì ±âÁ¸ÀÇ °ªÀ» À¯ÁöÇÕ´Ï´Ù\n");
			IO.print(format("°íÀ¯¹øÈ£ : {0}\n", target->id));
			IO.print(format("Á¦¸ñ : {0}\n", target->title));
			IO.print(format("Àå¸£ : {0}\n", target->genre));
			IO.print(format("ÀúÀÚ : {0}\n", target->author));
			IO.print(format("°¡°Ý : {0}\n", target->price));
...
#pragma endregion
	...
	menu_manager->AppendMenu(MENU_B_PRODUCT_BUY, new Menu<Product*>(
		[=](MenuIO& IO, Product* target) {
			...
			while (true) {
				input = IO.input(format("»ç¿ëÇÏ½Ç ÄíÆù °³¼ö¸¦ ÀÔ·ÂÇÏ¼¼¿ä\nº¸À¯ ÄíÆù °³¼ö : {0}°³",user->coupon_count));
	...
	menu_manager->AppendMenu(MENU_B_REFUND, new Menu<Invoice*>(
		[=](MenuIO& IO, Invoice* target) {
			...
			if (input == "y") {
				...
				IO.print("ÇØ´ç »óÇ°ÀÌ ¹ÝÇ°µÇ¾ú½À´Ï´Ù.");
				IO.pause();
