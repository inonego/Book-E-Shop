...
{
	// °¢ ¸Þ´º¿¡¼­ µÚ·Î°¡±â¸¦ À§ÇÑ ÀÌÀü ¸Þ´º¸¦ ¼³Á¤ÇÏ´Â °ÍÀ¸·Î ¿¢¼¿·Î °ü¸®µË´Ï´Ù.
		...
		[&](MenuIO& IO) { 
			...
			while (true) {
				...
				if ((user = shop_manager.Login(id, password)) != nullptr) {
					...
					IO.print("ÀÏÄ¡ÇÏ´Â °èÁ¤ Á¤º¸¸¦ Ã£À» ¼ö ¾ø½À´Ï´Ù.\n");
					IO.pause();
...
#pragma region ±¸¸ÅÀÚ ¸Þ´ºÈ­¸é
	...
	{
		TemplateMenuSelection _template;
		_template.SubMenu("»óÇ° °ü¸® (µî·Ï/¼öÁ¤/Á¦°Å, Àç°í°ü¸®)", []() { MENU_B_PRODUCT_LIST, shop_manager.GetProdcutList(); });
		_template.SubMenu("°í°´ °èÁ¤ Á¤º¸", []() { menu_manager.RunMenu(MENU_B_ACCOUNT_INFO); });
