...
#pragma region °ü¸®ÀÚ ¸Þ´ºÈ­¸é
	...
	// »óÇ° ¸ñ·Ï ¸Þ´ºÈ­¸é
	{
		...
		_template.show_func = [](Product* product) -> string {
			return format("{0:<10}{1:<20}{2:<10}{3:<10}{4:<12}{5:<8}", product->id, limit(product->title, 18), product->genre, limit(product->author, 8), to_string(product->price) + "¿ø", product->count);
		};
	...
	// ±¸¸Å ³»¿ª ¸Þ´ºÈ­¸é
	{
		...
		_template.header_func = []() -> string {
			return format("{0:<12}{1:<20}{2:<10}{3:<12}{4:<10}", "±¸¸Å ³¯Â¥", "Á¦¸ñ", "ÀúÀÚ", "°áÁ¦ ±Ý¾×", "»óÅÂ");
		};
