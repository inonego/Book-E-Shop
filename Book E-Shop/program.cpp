...
#pragma region ½ÃÀÛ ¸Þ´ºÈ­¸é
	menu_manager->AppendMenu(MENU_DATE, new Menu([=](MenuIO& IO) {
		...
		IO.print("ÇöÀç ³¯Â¥(YY.MM.DD Çü½Ä)¸¦ ÀÔ·ÂÇÏ¼¼¿ä.");
		IO.print(format("(³¯Â¥´Â {0} ´çÀÏ ¶Ç´Â ±× ÀÌÈÄ¿©¾ß ÇÕ´Ï´Ù.)", date_to_string(min_date)));
...
#pragma region °ü¸®ÀÚ ¸Þ´ºÈ­¸é
	...
	// ±¸¸Å ³»¿ª ¸Þ´ºÈ­¸é
	{
		...
		_template.header_func = []() -> string {
			return format("{0:<12}{1:<20}{2:<10}{3:<12}{4:<18}", "±¸¸Å ³¯Â¥", "Á¦¸ñ", "ÀúÀÚ", "°áÁ¦ ±Ý¾×", "»óÅÂ");
		};
		_template.show_func = [](int id) -> string {
			...
			return format("{0:<12}{1:<20}{2:<10}{3:<12}{4:<18}", limit(date_to_string(invoice->date),10), limit(product->title, 18), limit(product->author, 8),
				limit(to_string(invoice->final_price), 10) + "¿ø", limit(invoice->GetState(),15));
		};
