...
{
	// ³¯Â¥
	data_manager->AppendParser("date", (new Parser())
		->set_label("³¯Â¥")
		->set_regex(LR"(^[0-9]{2}.[0-9]{2}.[0-9]{2}$)")
		->set_msg_error("³¯Â¥ Çü½ÄÀÌ ¿Ã¹Ù¸£Áö ¾Ê½À´Ï´Ù.\n")
	);
...
#pragma region ½ÃÀÛ ¸Þ´ºÈ­¸é
	menu_manager->AppendMenu(MENU_DATE, new Menu([=](MenuIO& IO) {
		menu_manager->ToggleCommand('q');
		menu_manager->PrintCommand(); 

		IO.print_line();
		IO.print_aligned_center("[ ÇöÀç ³¯Â¥ ÀÔ·Â ]"); 

		chrono::system_clock::time_point min_date = shop_manager->GetLastDate();

		IO.print_aligned_center("ÇöÀç ³¯Â¥(YY.MM.DD Çü½Ä)¸¦ ÀÔ·ÂÇÏ¼¼¿ä.");
		IO.print_aligned_center(format("(³¯Â¥´Â {0} ´çÀÏ ¶Ç´Â ±× ÀÌÈÄ¿©¾ß ÇÕ´Ï´Ù.)", date_to_string(min_date)));
		...
		while (true) {
			...
			{
				IO.print("³¯Â¥´Â ÁÖ¾îÁø ³¯Â¥ ´çÀÏ ¶Ç´Â ±× ÀÌÈÄ¿©¾ß ÇÕ´Ï´Ù.\n");
				IO.pause();
			...
			while (true) {
				...
				if ((user = shop_manager->Login(id, password)) != nullptr) {
					if (user == shop_manager->GetAdminAccount()) {
						...
						menu_manager->SetCommonHeader([=](MenuIO& IO) {
							IO.print_aligned_center(format("{}´ÔÀÌ Á¢¼ÓÇÏ¿´½À´Ï´Ù.", limit(shop_manager->GetCurrentAccount()->name,60)));
						});
...
#pragma region °ü¸®ÀÚ ¸Þ´ºÈ­¸é
	...
	// »óÇ° ¸ñ·Ï ¸Þ´ºÈ­¸é
	{
		...
		_template.header_func = []() -> string {
			return format("{0:<10}{1:<20}{2:<10}{3:<20}{4:<12}{5:<8}", "ID", "»óÇ°", "Àå¸£", "ÀúÀÚ", "°¡°Ý", "Àç°í");
		};
		_template.show_func = [](Product* product) -> string {
			return format("{0:<10}{1:<20}{2:<10}{3:<20}{4:<12}{5:<8}", format("{0:06}",product->id), limit(product->title, 18), limit(product->genre,8) , limit(product->author, 18), limit(to_string(product->price),10) + "¿ø", limit(to_string(product->count),6));
		};
	...
	// °í°´ ¸ñ·Ï ¸Þ´ºÈ­¸é 
	{
		...
		_template.header_func = []() -> string {
			return format("{0:<16}{1:<16}{2:<16}", "¾ÆÀÌµð", "ÀÌ¸§", "ÀüÈ­¹øÈ£");
		};
	...
	menu_manager->AppendMenu(MENU_ACCOUNT_INFO, new Menu<Account*>(
		[=](MenuIO& IO, Account* target) {
			...
			IO.print_aligned_center("ÁÖ¹® Ã³¸® Á¤º¸(O)     ¼öÁ¤(M)      ÄíÆù Á¤º¸(C)");
	...
	// ±¸¸Å ³»¿ª ¸Þ´ºÈ­¸é
	{
		...
		_template.header_func = []() -> string {
			return format("{0:<12}{1:<20}{2:<10}{3:<12}{4:<19}", "±¸¸Å ³¯Â¥", "Á¦¸ñ", "ÀúÀÚ", "°áÁ¦ ±Ý¾×", "»óÅÂ");
		};
		_template.show_func = [](int id) -> string {
			...
			return format("{0:<12}{1:<20}{2:<10}{3:<12}{4:<19}", limit(date_to_string(invoice->date),10), limit(product->title, 18), limit(product->author, 8),
				limit(to_string(invoice->final_price), 10) + "¿ø", invoice->GetState());
		};
	...
	menu_manager->AppendMenu(MENU_INVOICE_INFO	, new Menu<int>(
		[=](MenuIO& IO, int target_id) {
			...
			if (shop_manager->IsAdmin() || target->state != PURCHASED) {
				...
				IO.print_line(false);
				IO.print_aligned_center("±¸¸Å È®Á¤(C)     ¹ÝÇ°(R)");
				IO.print_line();
				...
				if (input.size() == 1) {
					...
					if (command == 'c') {
						menu_manager->RunMenu(MENU_B_CONFIRM, target);
					}
					else if (command == 'r') {
	...
	// ÄíÆù ¸ñ·Ï ¸Þ´ºÈ­¸é
	{
		TemplateTable<chrono::system_clock::time_point> _template;
		_template.SetName("ÄíÆù ¸ñ·Ï");
		_template.header_func = []() -> string {
			return format("{0:<12}{1:<12}{2:<12}", "ÄíÆù", "¹ß±Þ ³¯Â¥", "¸¸·á ³¯Â¥");
		};
		_template.show_func = [](chrono::system_clock::time_point date) -> string {
			return format("{0:<12}{1:<12}{2:<12}", "3000¿ø", date_to_string(date), date_to_string(date + std::chrono::days(shop_manager->COUPON_EXPIRE_DATE)));
		};

		_template.next_menu_code = MENU_NONE;

		_template.Apply(MENU_ACCOUNT_COUPON);
	}
	#pragma endregion
...
#pragma region ±¸¸ÅÀÚ ¸Þ´ºÈ­¸é
	// ±¸¸ÅÀÚ ¸Þ´ºÈ­¸é
	{
		...
		_template.SubMenu("ÃÖ±Ù º» »óÇ° ¸ñ·Ï", [=]() { menu_manager->RunMenu(MENU_B_PRODUCT_RECENT, shop_manager->GetCurrentAccount()->recent_product_id_list); });
...
#pragma endregion
	...
	menu_manager->AppendMenu(MENU_B_PRODUCT_BUY, new Menu<Product*>(
		[=](MenuIO& IO, Product* target) {
			...
			while (true) {
				...
				IO.print_aligned_center(format("º¸À¯ ÄíÆù °³¼ö : {0}°³", user->coupon_list.size()));

				string input = IO.input();
				if (data_manager->GetParser("account_coupon_count")->Check(input)) {
					coupon_count = stoi(input);
					if (int(user->coupon_list.size()) >= coupon_count)
					{				
	...
	// ÃÖ±Ù º» »óÇ° ¸ñ·Ï ¸Þ´ºÈ­¸é
	{
		TemplateTable<int> _template;
		_template.SetName("ÃÖ±Ù º» »óÇ° ¸ñ·Ï");
	...
	menu_manager->AppendMenu(MENU_B_CONFIRM, new Menu<Invoice*>([=](MenuIO& IO, Invoice* target) {
		...
		IO.print_line();
		IO.print_aligned_center("[ ±¸¸Å È®Á¤ ]");
		IO.print_aligned_center("±¸¸Å È®Á¤ÇÏ½Ã°Ú½À´Ï±î? (y / n)");
		IO.print_aligned_center("(±¸¸Å È®Á¤À» Ãë¼ÒÇÒ ¼ö ¾øÀ¸¸ç ÀÌÈÄ ¹ÝÇ°À» ÇÒ ¼ö ¾ø½À´Ï´Ù.)");
