...
#pragma region °ü¸®ÀÚ ¸Þ´ºÈ­¸é
	...
	// »óÇ° ¸ñ·Ï ¸Þ´ºÈ­¸é
	{
		...
		_template.show_func = [](Product* product) -> string {
			return format("{0:<10}{1:<20}{2:<10}{3:<10}{4:<12}{5:<8}", format("{0:06}",product->id), limit(product->title, 18), product->genre, limit(product->author, 8), to_string(product->price) + "¿ø", product->count);
		};
