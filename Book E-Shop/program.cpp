...
#pragma region °ü¸®ÀÚ ¸Þ´ºÈ­¸é
	...
	// »óÇ° ¸ñ·Ï ¸Þ´ºÈ­¸é
	{
		...
		_template.header_func = []() -> string {
			return format("{0:<10}{1:<20}{2:<10}{3:<10}{4:<12}{5:<8}", "ID", "»óÇ°", "Àå¸£", "ÀúÀÚ", "°¡°Ý", "Àç°í");
		};
		_template.show_func = [](Product* product) -> string {
			return format("{0:<10}{1:<20}{2:<10}{3:<10}{4:<12}{5:<8}", format("{0:06}",product->id), limit(product->title, 18), limit(product->genre,8) , limit(product->author, 8), limit(to_string(product->price),10) + "¿ø", limit(to_string(product->count),6));
		};
